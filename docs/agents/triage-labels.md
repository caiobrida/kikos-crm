...
This repo tracks issues as local markdown (see `issue-tracker.md`), so there are no tracker labels — the role string is written on the `Status:` line near the top of each issue file, e.g. `Status: ready-for-agent`.
